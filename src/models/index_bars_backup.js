...
// db.collection('billings').find({billing_cycle: cycleDate,start_date:startDate1,end_date:EndDate1 })
// .toArray((error, user) => {
//   console.log(user);
// })
})
    ...
    if (req.file.originalname === 'invalid-end-date-txt.txt'){
       const dataBufferEndDate = fs.readFileSync('bars_files/invalid-end-date-txt.txt',{encoding:'utf8', flag:'r'});
        ...
        if (req.file.originalname === 'invalid-end-date-csv.csv'){
             ...
             const EndDate = myStringEndDate.split("\r\n")
              ...
              EndDate.forEach((Endday1) => {
                EndDate1 = Endday1.slice(13,23)
                  ...
                  $counter++;
                  const monthSpecial = EndDate1(0,1)
          ...
          //   input: fs.createReadStream('bars_files/billing-cycle-not-on-range-csv.csv')
          // });
          ...
          // if (req.file.filename === 'billing-cycle-not-on-range-csv.csv'){
          //   $counter = 0;
          //   arr.forEach((entry2) => {
          //       cycleDate2 = entry2.slice(0,1)
          //          $counter++;
          //           console.log(cycleDate2);
          //           if (cycleDate2 > 12) {
          //           //res.send ('Billing Cycle not on range at row ' + $counter);
          //            return cb(new Error('Billing Cycle not on range at row ' + $counter))
          ...
          //       const dataBufferValid = fs.readFileSync('bars_files/valid-txt.txt',{encoding:'utf8', flag:'r'});
          //       const myStringValid = dataBufferValid
          //       const ValidDate = myStringValid.split("\r\n")
          //      // console.log(ValidDate);
          //       $counter = 0;
          //       ValidDate.forEach((Validday) => {
          //         ValidDate1 = Validday.slice(0,2)
          //         //ValidSdate = Validday.slice(2,6)
          //           $counter++;
          //         //console.log(ValidDate1);
          //         ValidDate1 === ValidDate1.toString()
          //     const db = client.db(databaseName)
          //     //console.log(db.collection('billings').find({billing_cycle: ValidDate1}));
        
          //     db.collection("billings").find({billing_cycle: ValidDate1},).toArray((error, user) => {
          //       //console.log(user);
          //     });
          //   })
          //  }) 
